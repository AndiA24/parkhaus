...
Wir haben uns zu Beginn alle alleine Gedanken zu der Aufgabenstellung, Features und Konzepten zur technischen Realisierung gemacht. Diese haben wir dann bei einem gemeinsamen Termin abgegelichen und uns auf Anforderungen an die Funktionalität, Bedinung sowie Architektur geeinigt. Anhand dessen haben wir Arbeitspakete für unser Kanban-Board erstellt, und uns auf die Arbeitsteilung geeinigt, sodass sie Arbeitspakete alleine ohne weitere Absprachen bearneitet werden können.

Im Zuge dessen haben wir uns auf den Ablauf der Simulation im Pseudocode geeinigt und die dafür notwendigen Funktionen definiert, anhand der Funktionsprototypen sowie der zugehörigen Doxygen-Comments haben wir klar definiert, welche Parameter die Funktion erhält, was sie macht und was sie zurück gbt. IM Vergleich zu diesem anfänlichen Entwurf mussten nur kleine Änderungen an einzelnen Funktionen vorenommen werden und die wesentlich Struktur blieb erhalten.

Um einen Überblick über den aktuellen Arbeitsstand sowie die erledigten Arbeiten zu bewahren haben wir für alle Aufgaben / Arbeitspakete entsprechende Branches auf GIT erstellt, wenn sie finlisiert wurden wurden sie in eine Pull-Request diskutiert. Hier haben wir gegebenenfalls Fehler korrigiert, Umsetzungen diskutiert oder angemerkt falls uns etwas eingefallen ist, was noch zu beachten ist oder was im Konflikt zu den eigenen Funktionen steht.

Jedes Teammitglied war zuständig und verantwortlich für Funktionen, zwar wurden auch manchmal gegenseitig Änderungen oder Anpassungen vorgenommen, doch hatte für jede Funktion, beziehungsweise jeden Abschnitt des Programms eine Person den Überblich über den Abvlauf und etwaiige To-Dos oder Fehler die durch neue Pull-Requests entstehen könnten.
...
Durch den konsequenten Einsatz von Pointern müssen Daten nicht kopiert / übergeben werden. Das bedeutet, aus den Funktionen kann direkt auf die Daten zugegriffen werden und sie können bearbeitet werden, indem lediglich ein Pointer darauf übergeben wird. Das spart Speicher unt steigert die Effizienz.

**Laufzeitoptimierungen:**

- Freie Parkplätze werden auf einem **Stack** gehalten → O(1) bei der Suche nach einem freien Platz (statt O(n) Iteration durch das gesamte Parkhaus)
...
Durch eine Optimierung, liegen freie Parkplätze und geparkte Fahrzeuge jeails auf einen Stack. Dadurch kann einfach der nächste freie Parkplatz genommen werden und es müssen nur die tatsächlich belegten Spots bei dem `check_exits()` durchlaufen werden. Das spart Zeit und Ressourcen.

**Error Handling:**

Ein später implementiertes Error-Hanlding baehandelt alle Fehler die zur Laufzeit auftreten können einheitlich.

Wir prüfen in jeder Funktion auf die Fehler, die auftreten können, wie zum Beispiel fehlende oder falsch übergebene Parameter, Fehler bei dem allokieren von Speicher, unplausible Werte oder die Division-by-zero. Dadurch können wir weitere Fehler erkennen bevor sie entstehen und Segaults oder Memeory-Leaks erkennen.

Wenn solche Fehler auftreten geben wir dir für C idiomatischen Rückgabewerte zurück: -1 / NULL für einen fatalen Fehler, -2 für sonstige Fehler, 0 für Erfolg. Nach jedem Funktionsaufruf prüfen wir den Rückgbewert, um im Falle eines Fehler reagieren zu können. Wir unterscheiden bewusst bei den Schweregraden der Fehler. Es gibt Fehler wie zum Beispiel in `create_initial_occupancy()` die dem Nutzer mitgeteilt werden, den weiteren Programmlauf aber nicht behindern, wodurch das Programm fortgesetzt werden kann. Andere Fehler, wie zum Beispiel Fehler bei dem Allokieren von Speicher, um die `Struct Parking` zu erstellen führen zu einem fatalen Fehler, daher wird hier das Programm gezielt beendet.

Wir haben für sämlichen allokierten Speicher clean-ups 7 clean-up Funktionen, um Memory-Leaks zu vermeiden. An manchen Stellen geben wir den Speicher jedoch nicht bewusst im Code frei sondnern verwenden den nativen Befehl `exit()`, der das Programm beendet und den gesammten im Heap allokierten Speicher freigibt.
...
Wir haben uns bereits zu Beginn in der Planung der Anforderungen und der Umsetzung entschieden, eingige Anforderungen zu setzen, die über die Aufgabenstellung hinausgehen.
...
- **Endlosschleife**: Das Programm läuft nach einer Simulation nicht automatisch aus, sondern ermöglicht mehrere aufeinanderfolgende Simulationen bis zur bewussten Beendigung
