...
| `get_config(ptr_config)` | Liest Simulationsparameter aus der Konfigurationsdatei in die Struct. Existiert keine Datei, werden Standardwerte verwendet. | `SimConfig *ptr_config` | `0` Standardwerte geladen, `1` Datei geladen, `-1` bei Fehler |
...
| Funktion | Beschreibung | Parameter | Rückgabe |
|---|---|---|---|
| `initial_occupancy(ptr_parking, ptr_config, ptr_stats)` | Befüllt die Parkhausstruktur mit der in der Konfiguration festgelegten Anzahl an Initialfahrzeugen. Bei Fehler wird der bereits allokierte Speicher freigegeben. | `Parking *ptr_parking`, `SimConfig *ptr_config`, `SimStats *ptr_stats` | `0` bei Erfolg, `-1` bei schwerem Fehler, `-2` bei zu wenig freien Plätzen |
| ***Begründung:*** | Trennt die initiale Befüllung vom Aufbau der Struktur, damit `init_parking` einen definierten Ausgangszustand erhält ohne selbst die Fahrzeugerzeugung zu kennen | | |
...
| `entry_parking(ptr_parking, ptr_vehicle, ptr_stats)` | Parkt ein Fahrzeug auf dem ersten freien Stellplatz und aktualisiert die Belegungszahl sowie die Simulationsstatistiken. | `Parking *ptr_parking`, `Vehicle *ptr_vehicle`, `SimStats *ptr_stats` | `1` bei Erfolg, `0` wenn kein freier Platz verfügbar, `-1` bei ungültigem Argument |
...
| `check_exit(ptr_parking, ptr_stats)` | Prüft alle belegten Stellplätze auf Fahrzeuge, deren Parkdauer abgelaufen ist, entfernt sie und aktualisiert Zähler und Statistiken. | `Parking *ptr_parking`, `SimStats *ptr_stats` | `1` bei Erfolg, `-1` bei ungültigem Argument |
...
| `free_parking(ptr_parking, ptr_stats)` | Gibt den gesamten durch `init_parking` allokierten Speicher frei. | `Parking *ptr_parking`, `SimStats *ptr_stats` | `0` bei Erfolg, `-1` bei Fehler |
...
| `enqueue(ptr_queue, ptr_vehicle, ptr_stats)` | Hängt ein Fahrzeug an das Ende der Warteschlange. Inkrementiert `size` um 1. | `Queue *ptr_queue`, `Vehicle *ptr_vehicle`, `SimStats *ptr_stats` | `0` bei Erfolg, `-1` bei ungültigem Argument |
...
| `delete_queue(ptr_queue, ptr_stats)` | Gibt alle Knoten der Warteschlange sowie die enthaltenen `Vehicle`-Objekte frei und gibt anschließend die `Queue`-Struct selbst frei (ruft intern `free_queue` auf). | `Queue *ptr_queue`, `SimStats *ptr_stats` | `0` bei Erfolg, `-1` bei ungültigem Argument |
| ***Begründung:*** | Kapselt das vollständige Aufräumen der Warteschlange am Simulationsende in einem einzigen Aufruf | | |
| `free_queue(ptr_queue)` | Gibt die durch `init_queue` allokierte `Queue` Struct frei. | `Queue *ptr_queue` | `0` bei Erfolg, `-1` bei ungültigem Argument |
...
| `create_output_file(ptr_config)` | Erstellt die CSV-Ausgabedatei (überschreibt eine bestehende Datei). Schreibt die Simulationskonfiguration und die Spaltenüberschriften als Header. | `SimConfig *ptr_config` | `FILE *` (offenes Datei-Handle) oder `NULL` bei Fehler |
...
| `update_simstats(ptr_stats, ptr_parking, ptr_queue)` | Sammelt und berechnet alle relevanten Kennwerte für den aktuellen Simulationsschritt und schreibt sie in die `SimStats` Struct. | `SimStats *ptr_stats`, `Parking *ptr_parking`, `Queue *ptr_queue` | `0` bei Erfolg, `-1` bei ungültigem Argument |
| ***Begründung:*** | Zentralisiert die Berechnung aller Schrittkennwerte | | |
| `update_peak(ptr_stats)` | Vergleicht temporäre Werte mit gespeicherten Höchstwerten und aktualisiert diese, falls ein neues Maximum erkannt wird, und speichert die aktuelle Schrittnummer. | `SimStats *ptr_stats` | `0` bei Erfolg, `-1` bei ungültigem Argument |
...
| `save_temp_dataset(ptr_stats, ptr_output_file)` | Schreibt die Statistiken des aktuellen Schritts in die Ausgabedatei. | `SimStats *ptr_stats`, `FILE *ptr_output_file` | `1` bei Erfolg, `-1` bei ungültigem Argument |
...
| `reset_temp_stats(ptr_stats)` | Setzt alle per-Step (temporären) Felder der Struct auf null zurück. | `SimStats *ptr_stats` | `0` bei Erfolg, `-1` bei ungültigem Argument |
...
| `save_final_dataset(ptr_stats, ptr_output_file)` | Schreibt die abschließende Simulationszusammenfassung in die Ausgabedatei. | `SimStats *ptr_stats`, `FILE *ptr_output_file` | `1` bei Erfolg, `-1` bei ungültigem Argument |
| ***Begründung:*** | Kapselt die abschließende Ausgabe, damit die Zusammenfassung konsistent an einer Stelle geschrieben wird | | |
| `close_output_file(ptr_output_file)` | Schließt die geöffnete Ausgabedatei. | `FILE *ptr_output_file` | `0` bei Erfolg, `-1` bei Fehler |
...
| `reset_all_stats(ptr_stats)` | Setzt alle Felder der Struct auf null zurück. | `SimStats *ptr_stats` | `0` bei Erfolg, `-1` bei ungültigem Argument |
...
| `free_stats(ptr_stats)` | Gibt den durch `init_simstats` allokierten Speicher frei. | `SimStats *ptr_stats` | `0` bei Erfolg, `-1` bei ungültigem Argument |
...
| `quit(ptr_config)` | Speichert die Konfiguration (falls `ptr_config != NULL`) und beendet das Programm via `exit(1)`. | `SimConfig *ptr_config` | — |
| ***Begründung:*** | Kapselt den Programmabschluss, damit die Konfigurationsspeicherung garantiert an einer zentralen Stelle erfolgt und nicht vom Aufrufer manuell durchgeführt werden muss | | |
